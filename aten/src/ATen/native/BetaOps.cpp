...
static Tensor _betaincinv_initial_approx(const Tensor& y, const Tensor& a, const Tensor& b, at::ScalarType& dtype) {
  ...
  Tensor eps = std::move(std::get<0>(eps_tiny_maxexp));
  Tensor tiny = std::move(std::get<1>(eps_tiny_maxexp));
  Tensor maxexp = std::move(std::get<2>(eps_tiny_maxexp));
  TensorOptions options = at::TensorOptions().dtype(dtype).device(y.device());
  Tensor one = at::tensor(1.0, options);
  Tensor two = at::tensor(2.0, options);
  Tensor three = at::tensor(3.0, options);
  Tensor five = at::tensor(5.0, options);
  Tensor six = at::tensor(6.0, options);
  Tensor max_log = (maxexp - one) * at::log(two);
...
static Tensor _betaincinv_computation(const Tensor& y, const Tensor& a, const Tensor& b) {
  ...
  TensorOptions options = at::TensorOptions().dtype(dtype).device(_y.device());
  Tensor eps = std::move(std::get<0>(eps_tiny_nan));
  Tensor tiny = std::move(std::get<1>(eps_tiny_nan));
  Tensor nan = std::move(std::get<2>(eps_tiny_nan));
  Tensor zero = at::tensor(0.0, options);
  Tensor half = at::tensor(0.5, options);
  Tensor one = at::tensor(1.0, options);
  Tensor two = at::tensor(2.0, options);
  Tensor halley_correction_min = at::tensor(0.5, options);
  Tensor halley_correction_max = at::tensor(1.5, options);
  Tensor _true = at::tensor(true, options);
  ...
  if (dtype == at::ScalarType::Float) {
      max_iterations = 10;
      tolerance = at::tensor(8.0, options) * eps;
  } else { // T is double
      max_iterations = 8;
      tolerance = at::tensor(4096.0, options) * eps;
  }
  ...
  result = (at::equal(_y, zero) || at::equal(_y, one)) ? _y: result;
